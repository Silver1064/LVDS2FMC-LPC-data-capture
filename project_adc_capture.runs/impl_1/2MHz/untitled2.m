...
data = readmatrix('10mv.csv');

% Lấy cột đầu tiên của dữ liệu
x = data(:, 4); % Trích xuất cột đầu tiên

% Tham số bộ lọc
f_low = 1.995e6;       % Tần số cắt thấp
f_high = 2.005e6;      % Tần số cắt cao
...
y = filter(b, a, x) * 29000 / 2^12;

% Ghi tín hiệu đã lọc ra file Excel
output_filename = 'filtered_signal_1MHz.xlsx';
writematrix(y, output_filename, 'Sheet', 2, 'Range', 'A2');
